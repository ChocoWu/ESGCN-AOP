...
class SynFueBERT(BertPreTrainedModel):
    ...
    def _forward_train(self, encodings: torch.tensor, context_masks: torch.tensor, term_masks: torch.tensor,
        ...
        h, dep_output = self.SpanEnc(input_ids=encodings, input_masks=context_masks, simple_graph=simple_graph,
                                     graph=graph)
        ...
        for i in range(0, relations.shape[1], self._max_pairs):
            ...
            chunk_rel_logits, chunk_rel_clf3, chunk_dep_score = self._classify_relations(span_repr,
                                                                                         term_spans_pool,
                                                                                         size_embeddings,
                                                                                         relations, rel_masks,
                                                                                         h_large, i,
